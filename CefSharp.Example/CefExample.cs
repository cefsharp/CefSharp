...
namespace CefSharp.Example
{
    public static class CefExample
    {
        ...
        public static void Init(string phoneNumber, string userProxy, CefSettingsBase settings, IBrowserProcessHandler browserProcessHandler)
        {
                if (!string.IsNullOrWhiteSpace(userProxy))
                {
                    var arr = userProxy.Split(new[] { ":" }, StringSplitOptions.RemoveEmptyEntries).ToList();
                    var ip = string.Empty;
                    ...
                    {
                        CefSharpSettings.Proxy = new ProxyOptions(ip: ip, port: port);
                    }
                    else
                    {
                        CefSharpSettings.Proxy = new ProxyOptions(ip: ip, port: port, username: userName, password: pass);
                    }
                }
                ...
                var rootPath = Path.Combine(AppContext.BaseDirectory, "cache");
                if (!Directory.Exists(rootPath))
                {
                    Directory.CreateDirectory(rootPath);
                }

                settings.RootCachePath = Path.Combine(rootPath, "_" + phoneNumber);

                //If non-null then CachePath must be equal to or a child of RootCachePath
                //We're using a sub folder.
                //
                var cachePath = Path.Combine(settings.RootCachePath, "global");
                settings.CachePath = Path.GetFullPath(cachePath);
...
#else
                    settings.BrowserSubprocessPath = Path.Combine(AppContext.BaseDirectory, "CefSharp.BrowserSubprocess.exe");
#endif
                }
                ...
                string cefSharpExampleResourcesFolder = Path.Combine(AppContext.BaseDirectory, "Resources2");
                ...
                if (!Cef.Initialize(settings, performDependencyCheck: performDependencyCheck))
                {
