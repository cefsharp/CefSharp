...
namespace CefSharp.Internals
{
    public static class ExceptionExtension
    {
        ...
        /// </summary>
        public const int INNER_EXCEPTION_LOG_COUNT = 5;
        ...
        [DebuggerStepThrough]
        public static String CreateDetailedExceptionString(this Exception ex, int limit = INNER_EXCEPTION_LOG_COUNT)
        {
            var innerException = ex.InnerException;
            if (innerException != null && limit > 0)
            {
                limit = limit - 1;
                return ex.Message + ":\n" + ex.StackTrace + "\ncaused by:\n" + innerException.CreateDetailedExceptionString(limit);
            }
            else
            {
                return ex.Message + ":\n" + ex.StackTrace;
