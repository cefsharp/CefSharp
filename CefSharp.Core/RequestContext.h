...
namespace CefSharp
{
	//class RequestContextHandler : public virtual CefRequestContext {
	//public:
	//	///
	...
	public:
		RequestContext(CookieManager^ cookieManager) : _cookieManager(cookieManager), _requestContext(&CefRequestContext::CreateContext(cookieManager->GetHandler())), _isFinalized(false) { }
		!RequestContext() { delete _requestContext; _cookieManager = nullptr; _isFinalized = true; }
		~RequestContext() { if (!_isFinalized) this->!RequestContext(); }
