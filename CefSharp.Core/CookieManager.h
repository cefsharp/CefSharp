...
namespace CefSharp
{
	...
	public:
		CookieManager(String^ cookiePath, bool persistSessionCookies) : _handler(new CefRefPtr<CefRequestContextHandler>((CefRequestContextHandler*)new RequestContextHandler(CefCookieManager::CreateManager(StringUtils::ToNative(cookiePath), persistSessionCookies)))), _isFinalized(false) { }
		!CookieManager() { delete _handler; _isFinalized = true; }
		~CookieManager() { if (!_isFinalized) this->!CookieManager(); }
